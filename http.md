...
**`getUsers($dataFile)`** — função definida em `data.php` que lê o arquivo `data.json` e retorna como array PHP.
Por dentro ela faz `json_decode(file_get_contents($dataFile), true)`.
O `$dataFile` foi definido em `config.php` como `__DIR__ . '/../data/data.json'`.

```php
getUsers($dataFile);
// Retorna: ['users' => [['name' => 'Victor', 'age' => 24]]]  ← é um ARRAY PHP
```

**`json_encode(...)`** — converte o array PHP de volta pra string JSON.

**`echo`** — "cospe" essa string pro cliente. O `echo` no PHP é o que envia dados na resposta HTTP.
...
Frontend:  fetch('http://localhost:8000/api/users')
                          ↓
Backend:   getUsers($dataFile)  → ['users' => [...]]
                          ↓
Backend:   echo json_encode(...)  → envia como resposta HTTP
                          ↓
...
─────────────────                    ─────────────
fetch(url)                    →      GET    → getUsers() → echo json_encode
fetch(url, {method:'POST'})   →      POST   → php://input → validate → saveUsers()
fetch(url, {method:'PUT'})    →      PUT    → php://input → validate → substituir tudo → saveUsers()
fetch(url, {method:'PATCH'})  →      PATCH  → php://input → array_merge (parcial) → saveUsers()
fetch(url, {method:'DELETE'}) →      DELETE → $_GET['index'] → array_splice → saveUsers()
